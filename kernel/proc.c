...
#define inf16 10000000000000000
...
int calculate_dp(long long previous_runtime, long long previous_sleeptime, int staticp, int ifitisnew_proc)
{
  int niceness;
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      acquire(&p->lock);
      int dp = calculate_dp(p->previous_runtime, p->previous_sleeptime, p->SP, p->new_proc); // we still have to calculate this
      if ((p->state == RUNNABLE) && (firsttime || dp < minPriority || (dp == minPriority && p->scheduled_ct < minSched) || (dp == minPriority && p->scheduled_ct == minSched && p->cTimee < minTime)))
      {
        if (firsttime != 1)
        {
    ...
    // to release its lock and then reacquire it
    // before jumping back to us.
    minProc->scheduled_ct++;
    minProc->state = RUNNING;
    c->proc = minProc;
    swtch(&c->context, &minProc->context);

    p->previous_runtime = p->previous_sleeptime = 0;
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      int current_wait_time = ticks - p->ptimeqenter - p->previous_runtime;

      if (p->state == RUNNABLE)
      {

        if (current_wait_time >= 200) // then implement starvation prevention
        {
          removeparticularfromq(&fivequeues[p->pcurrentqlevel], p);

          p->pcurrentqlevel--;
          if (p->pcurrentqlevel == -1)
            p->pcurrentqlevel = 0;
        }

        if (p->isthispinq == 0)
        {
          addtoq(&fivequeues[p->pcurrentqlevel], p);
        }
      }
    ...
    int found = 0;
    for (int loopthroughallq = 0; loopthroughallq < 5; loopthroughallq++)
    {
      for (int throughthisq = 0; throughthisq < 64; throughthisq++)
      {
        if (queueesizee(&fivequeues[loopthroughallq]) == 0)
          break;

        struct proc *cur = fivequeues[loopthroughallq].allProcesses[fivequeues[loopthroughallq].first];

        acquire(&cur->lock);

        if (cur->state != RUNNABLE)
        {
          removeparticularfromq(&fivequeues[loopthroughallq], cur);
          release(&cur->lock);
          continue;
        }
    ...
    c->proc = p;
    p->state = RUNNING;
    p->checkfornextq = (1 << p->pcurrentqlevel);
    swtch(&c->context, &p->context);
    p->ptimeqenter = ticks;
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        totalticketcnt += p->numberoftickets;
      }
      release(&p->lock);
    }

    int lotteryticket = (getrandomnumber() % totalticketcnt);
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        currentsum += p->numberoftickets;

        if (currentsum > lotteryticket)
        {
          found = p;
          break;
        }
...
void set_priority(int newp, int tochangepid)
{
  ...
  for (struct proc *p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->pid == tochangepid)
    {
      ...
      // printf("The priority of the process has been successfully changed\n");
    }
    release(&p->lock);
  }
