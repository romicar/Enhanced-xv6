...
void usertrap(void)
{
  ...
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING)
  {
      ...
      if(myproc()->checkfornextq <= 0)
      {
        myproc()->pcurrentqlevel++;
        if(myproc()->pcurrentqlevel == 5) myproc()->pcurrentqlevel = 4;
        changee = 1; 
      }
      if(changee) yield();
      ...
      for(int i = 0; i < myproc()->pcurrentqlevel; i++)
      {
        if(queueesizee(&fivequeues[i]) > 0) changee = 1; 
      }
